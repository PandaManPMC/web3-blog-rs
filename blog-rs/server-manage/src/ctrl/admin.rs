...
async fn login(
    headers: HeaderMap,
    Json(payload): Json<bean::admin::LoginIn>
) -> Result<Json<common::net::rsp::Rsp<bean::admin::LoginOut>>, app_err::AppError> {
    ...
    if author_res.is_err() {
        return Ok(Json(common::net::rsp::Rsp::<bean::admin::LoginOut>::err_de()))
    }
    ...
    if let Some(au) = author {
        let pwd = plier::md::sha256(payload.user_pwd);
        if pwd != au.user_pwd {
            return Ok(Json(common::net::rsp::Rsp::<bean::admin::LoginOut>::fail("username or password incorrect".to_string())))
        }

        // google 验证器校验
        if plier::str::is_not_blank(au.google_auth_secret) {
            if plier::str::is_blank(payload.google_auth_code) {
                return Ok(Json(common::net::rsp::Rsp::<bean::admin::LoginOut>::fail("google 验证码错误".to_string())))
            }
            // ---- 进行校验
